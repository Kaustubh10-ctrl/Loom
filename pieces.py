...
class Piece():
    ...
    def _init_(self, x, y, color, piece_type, value):
        self.x = x
        self.y = y
        self.color = color
    ...
    def get_possible_diagonal_moves(self, board):
        ...
        for i in range(1, 8):
            ...
            piece = board0.get_piece(self.x+i, self.y+i)
            moves.append(self.get_move(board, self.x+i, self.y+i))
            if (piece != 0):
                break

        for i in range(1, 8):
            if (not board0.in_bounds(self.x+i, self.y-i)):
                break
    ...
    def get_possible_horizontal_moves(self, board):
        ...
        for i in range(1, 8 - self.x):
            piece = board0.get_piece(self.x + i, self.y)
        ...
        for i in range(1, 8 - self.y):
            piece = board0.get_piece(self.x, self.y + i)
    ...
    def get_move(self, board, xto, yto):
        move = 0
        if (board0.in_bounds(xto, yto)):
            piece = board0.get_piece(xto, yto)
            if (piece != 0):
...
class Rook(Piece):
    ...
    def _init_(self, x, y, color):
        super(Rook, self)._init_(x, y, color, Rook.PIECE_TYPE, Rook.VALUE)
...
class Knight(Piece):
    ...
    def _init_(self, x, y, color):
        super(Knight, self)._init_(x, y, color, Knight.PIECE_TYPE, Knight.VALUE)
...
class Bishop(Piece):
    ...
    def _init_(self, x, y, color):
        super(Bishop, self)._init_(x, y, color, Bishop.PIECE_TYPE, Bishop.VALUE)
...
class Queen(Piece):
    ...
    def _init_(self, x, y, color):
        super(Queen, self)._init_(x, y, color, Queen.PIECE_TYPE, Queen.VALUE)
...
class King(Piece):
    ...
    def _init_(self, x, y, color):
        super(King, self)._init_(x, y, color, King.PIECE_TYPE, King.VALUE)
    ...
    def get_top_castling_move(self, board):
        ...
        piece = board0.get_piece(self.x, self.y-3)
        if (piece != 0):
            if (piece.color == self.color and piece.piece_type == Rook.PIECE_TYPE):
                if (board0.get_piece(self.x, self.y-1) == 0 and board0.get_piece(self.x, self.y-2) == 0):
                    return ai.Move(self.x, self.y, self.x, self.y-2, True)

        return 0

    def get_bottom_castling_move(self, board):
        if (self.color == Piece.WHITE and board0.white_king_moved):
            return 0
        if (self.color == Piece.BLACK and board0.black_king_moved):
            return 0

        piece = board0.get_piece(self.x, self.y+4)
        if (piece != 0):
            if (piece.color == self.color and piece.piece_type == Rook.PIECE_TYPE):
                if (board0.get_piece(self.x, self.y+1) == 0 and board0.get_piece(self.x, self.y+2) == 0 and board0.get_piece(self.x, self.y+3) == 0):
                    return ai.Move(self.x, self.y, self.x, self.y+2, True)
...
class Pawn(Piece):
    ...
    def _init_(self, x, y, color):
        super(Pawn, self)._init_(x, y, color, Pawn.PIECE_TYPE, Pawn.VALUE)
    ...
    def get_possible_moves(self, board):
        ...
        if (board0.get_piece(self.x, self.y+direction) == 0):
            moves.append(self.get_move(board, self.x, self.y + direction))

        # The Pawn can take 2 steps as the first move.
        if (self.is_starting_position() and board0.get_piece(self.x, self.y+ direction) == 0 and board0.get_piece(self.x, self.y + direction*2) == 0):
            moves.append(self.get_move(board, self.x, self.y + direction * 2))

        # Eating pieces.
        piece = board0.get_piece(self.x + 1, self.y + direction)
        if (piece != 0):
